...

{
	...
	for (ch = 0; ch < ch_num; ch++)
	{
		...
		*/
		...
		/* LSP quantization */
		/* ---------------- */
		//Qua_lspe(lsp_out_new, lsp_out_new_q, code_lsp, freq_prev, freq_cur);
		...
		printf("------LSF_out------\n");
#endif
		//Int_qlpc(lsp_out_old_q, lsp_out_new_q, A_t_fwd_q); //Convert LSP to LPC coeff. (Out Q12)
		//Int_qlpc(lsp_out_old, lsp_out_new, A_t_fwd); //Convert LSP to LPC coeff. (Out Q12)
		Int_qlpc(lsp_out_old, lsp_out_new, &A_t_fwd[0]); //Convert LSP to LPC coeff. (Out Q12)
		//Int_lpc(lsp_out_old, lsp_out_new, lsf_out_int, lsf_out_new, A_t_fwd);
#if UPMIX_DEBUG == 1
		for (int z = 0; z < MP1; z++)
		{
			printf("%f\n", A_out_t[z]);
		...
		//LP_env_log(A_t_fwd_q, LP_Mag_dec);
