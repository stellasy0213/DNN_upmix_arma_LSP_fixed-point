...
/*
 File : lpccp.c
 */
/* from lpce.c G.729 Annex E Version 1.2  Last modified: May 1998 */
/* from lpc.c G.729 Annex B Version 1.3  Last modified: August 1997 */
/* from lpc.c G.729 Version 3.3            */
...
)
{
    ...
    for(i=1; i<=m; i++)
    {
        x  = Mpy_32(r_h[i], r_l[i], lag_h_B[i-1], lag_l_B[i-1]);
...
)
{
    ...
    short Ah[M_BWDP1], Al[M_BWDP1];      /* LPC coef. in double prec.                   */
    short Anh[M_BWDP1], Anl[M_BWDP1];    /* LPC coef.for next iteration in double prec. */
    ...
    t0 = L_sub( (int)0x7fffffffL, t0 ); /* 1 - K*K  in Q31 */
    ...
    *Err = shr(alp_h, alp_exp);
    /* Truncate A[i] in Q27 to Q12 with round_fixeding */
    
    A[0] = 4096;
    for(i=1; i<=m; i++)
    {
        t0   = L_Comp(Ah[i], Al[i]);
        old_A[i] = A[i] = round_fixed(L_shl(t0, 1));
    }
    old_rc[0] = rc[0];
    old_rc[1] = rc[1];
...
static short Chebps_11(short x, short f[], short n);
static short Chebps_10(short x, short f[], short n);
...
{
    short i, j, nf, ip;
    short xlow, ylow, xhigh, yhigh, xmid, ymid, xint;
    short x, y, sign, exp;
    short *coef;
    short f1[M/2+1], f2[M/2+1];
    int t0, L_temp;
    Flag   ovf_coef;
    short (*pChebps)(short x, short f[], short n);
...
static short Chebps_11(short x, short f[], short n)
{
    ...
    for (i = 2; i<n; i++)
    {
        ...
        t0 = L_shl(t0, 1);
        t0 = L_mac(t0,b2_h,(short)-32768L); /* t0 = 2.0*x*b1 - b2         */
        t0 = L_msu(t0, b2_l, 1);
    ...
    t0 = L_mac(t0, b2_h,(short)-32768L);  /* t0 = x*b1 - b2          */
    t0 = L_msu(t0, b2_l, 1);
