...

{
	...
	for (ch = 0; ch < ch_num; ch++)
	{
		...
		*/
		...
		for (k = 0; k < NNET_UPMIX_FFT21_LEN; ++k)
		{
			d_re = (NNET_UPMIX_FFT_freq_i[k].r * NNET_UPMIX_FFT_freq_i[k].r) << 6; //FFT Fraction: Q16.16
			d_im = (NNET_UPMIX_FFT_freq_i[k].i * NNET_UPMIX_FFT_freq_i[k].i) << 6;
			*(NNET_UPMIX_FFT_mag_i + k) = fix16_sqrt(d_re + d_im); //abs
			*(NNET_UPMIX_FFT_angl_i + k) = fix16_atan2(d_im, d_re);	//angle
		}
		...
		*/
		...
		for (int z = 0; z < NNET_UPMIX_LPCOEFF; z++)
		{
			lsf_out_new[z] = (p_i_LSF_dec[z] * PI_Q3) >> 3; //Denormalization, (Q13)
		}
		...
		*/
		...
		for (k = 0; k < NNET_UPMIX_FEAT_LEN; ++k)
		{
			...
			NNET_UPMIX_FFT_mag_out_i[k] = fix16_exp(*(LP_Mag_dec + k) + EPSILON * (log_val>>2) );
		}
		...
		for (k = 0; k < NNET_UPMIX_FEAT_LEN; ++k)
		{
			NNET_UPMIX_FFT_mag_out_i[k] = ETA * NNET_UPMIX_FFT_mag_out_i[k]>>5 + (32 - ETA) * NNET_UPMIX_FFT_mag_i[k]>>5;
			NNET_UPMIX_FFT_mag_out_i[k] = (1 - ETA) * NNET_UPMIX_FFT_mag_out_i[k]>>5;
		}
//==================================================================================================
//==================================================================================================
		...
		for (k = 0; k < NNET_UPMIX_FFT21_LEN; ++k)
		{
			NNET_UPMIX_FFT_freq_i[k].r = fix16_mul(NNET_UPMIX_FFT_mag_out_i[k], fix16_cos(NNET_UPMIX_FFT_angl_i[k])) >> 11; //Q.5
			NNET_UPMIX_FFT_freq_i[k].i = fix16_mul(NNET_UPMIX_FFT_mag_out_i[k], fix16_sin(NNET_UPMIX_FFT_angl_i[k])) >> 11;
...
void LP_env_log(short * A, int * env) //{input: Q20.12, output: Q16.16}
{
	...
	{
		//get 1/freq_A[k] (Complex number inverse)
		d_re = (freq_A[k].r * freq_A[k].r) << 6; //d_re: Q16.16
		d_im = (freq_A[k].i * freq_A[k].i) << 6;
		d_pow = d_re + d_im + FLR; //abs

		d_re = fix16_div(d_re, d_pow);
		d_im = fix16_div(d_im, d_pow);
		d_re = fix16_mul(d_re, d_re);
		d_im = fix16_mul(d_im, d_im);


		//Get envelope of d_im
		env[k] = fix16_log(fix16_sqrt(d_re + d_im)); //abs & log
	}
