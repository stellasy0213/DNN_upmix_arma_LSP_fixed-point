...
)
{
  ...
  for(i= m-(short)1; i >= 0; i--)
  {
    ...
    tmp = round_fixed(L_shl(L_tmp, 3));     /*(lsp[i]-table[ind])*slope[ind])>>12*/
...
)
{
  ...
  for(i=1; i<m; i++)
  {
    ap[i] = round_fixed( L_mult(a[i], fac) );
    fac   = round_fixed( L_mult(fac, gamma) );
  }
  ap[m] = round_fixed( L_mult(a[m], fac) );
...
void Int_lpc(
 short lsp_old[], /* input : LSP vector of past frame              */
 short lsp_new[], /* input : LSP vector of present frame           */
 short lsf_int[], /* output: interpolated lsf coefficients         */
 short lsf_new[],
 short Az[]       /* output: interpolated Az() for the 2 subframes */
)
{
  ...
  //Lsp_lsf2(lsp, lsf_int, M);      /* transformation from LSP to LSF (freq.domain) */
  //Lsp_lsf2(lsp_new, lsf_new, M);  /* transformation from LSP to LSF (freq.domain) */
  return;
}
